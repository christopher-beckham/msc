...
def get_net(args):
    ...
    l_conv3 = Conv2DLayer(
        l_conv2, num_filters=64, filter_size=(3,3), nonlinearity=nonlinearity)
    l_mp1 = MaxPool2DLayer(l_conv3, pool_size=(2,2))
    l_conv4 = Conv2DLayer(
        l_mp1, num_filters=96, filter_size=(3,3), nonlinearity=nonlinearity)
    l_conv5 = Conv2DLayer(
        l_conv4, num_filters=128, filter_size=(3,3), nonlinearity=nonlinearity)
    l_conv6 = Conv2DLayer(
        l_conv5, num_filters=128, filter_size=(3,3), nonlinearity=nonlinearity)
    l_out = l_conv6
