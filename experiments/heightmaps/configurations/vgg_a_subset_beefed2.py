...
def get_net(args):
    ...
    l_conv2 = Conv2DLayer(
        l_conv1, num_filters=64, filter_size=(3,3), nonlinearity=nonlinearity)
    l_mp1 = MaxPool2DLayer(l_conv2, pool_size=(2,2))
    l_conv3 = Conv2DLayer(
        l_mp1, num_filters=128, filter_size=(3,3), nonlinearity=nonlinearity)
    l_conv4 = Conv2DLayer(
        l_conv3, num_filters=128, filter_size=(3,3), nonlinearity=nonlinearity)
    l_mp2 = MaxPool2DLayer(l_conv4, pool_size=(2,2))
    l_conv5 = Conv2DLayer(
        l_mp2, num_filters=256, filter_size=(3,3), nonlinearity=nonlinearity)
    l_conv6 = Conv2DLayer(
        l_conv5, num_filters=256, filter_size=(3,3), nonlinearity=nonlinearity)
    l_conv7 = Conv2DLayer(
        l_conv6, num_filters=256, filter_size=(3,3), nonlinearity=nonlinearity)
    l_out = l_conv7
